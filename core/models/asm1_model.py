...
params_vlg = {
    ...
    'Y': 0.67,           # Урожайность гетеротрофов, г ХПК/г ХПК
                         # Стандартное значение ASM1, подходит для любых городов [citation:1]
    
    'mu_max': 3.5,       # Максимальная скорость роста гетеротрофов, 1/день
                         # Снижено с 6.0 для учёта высокой нагрузки Волгограда
    
    'K_S': 120.0,        # Константа полунасыщения по субстрату, мг ХПК/л
                         # Повышено (обычно 20) из-за сложного состава стоков
                         # В крупных промцентрах органика хуже усваивается
    
    'b': 0.15,           # Скорость эндогенного дыхания (отмирания), 1/день
                         # Чуть выше стандартного (0.62), т.к. при высокой нагрузке
                         # ил быстрее теряет активность
    
    'K_O': 0.5,          # Константа полунасыщения по кислороду, мг O₂/л
                         # Стандартное значение для гетеротрофов
    
    'X_max': 8000,       # Максимальная концентрация ила, мг/л
                         # Ограничение роста, чтобы ил не рос бесконечно
    
    # ============= ГИДРАВЛИЧЕСКИЕ ПАРАМЕТРЫ =============
    'V': 5000,           # Объём аэротенка, м³
                         # Типовой объём одной секции для крупного города
    
    'Q': 1000,           # Расход сточных вод, м³/ч
                         # Принят как расход на одну секцию (для масштаба города)
    
    # ============= ВРЕМЯ УДЕРЖАНИЯ =============
    'HRT': 5.0,          # Гидравлическое время удержания, ч (V/Q = 5 ч)
                         # Рассчитывается автоматически из V и Q
    
    'SRT': 10.0,         # Время удержания ила, дней
                         # Стандартное значение для стабильной работы
    
    # ============= КОНЦЕНТРАЦИИ НА ВХОДЕ (ПО ВОЛГОГРАДУ) =============
    'S_in': 250.0,       # Общий БПК на входе, мг/л
                         # Повышено по сравнению с Волжским (182.5)
                         # Данные Росприроднадзора: превышение ПДК более чем в 65 раз
    
    'S_bio_in': 200.0,   # Биодеградируемый субстрат, мг/л (80% от S_in)
                         # Увеличено пропорционально общей нагрузке
    
    'S_inert_in': 50.0,  # Инертный субстрат, мг/л (20% от S_in)
                         # Увеличено для учёта трудноокисляемой органики
    
    'X_in': 3000.0,      # Входная концентрация биомассы, мг/л
                         # Типовое значение для возвратного ила
    
    'O_in': 1.5,         # Кислород на входе, мг/л
                         # Типичная концентрация в поступающей воде
    
    # ============= ПАРАМЕТРЫ АЭРАЦИИ =============
    'kLa': 25.0,         # Коэффициент массопередачи кислорода, 1/день
                         # Снижен по сравнению с 35–40, т.к. при высокой нагрузке
                         # требуется меньше аэрации для избегания вспухания ила
    
    'O_sat': 8.0,        # Насыщающая концентрация кислорода, мг/л
                         # Зависит от температуры (стандарт)
    
    # ============= РЕЦИРКУЛЯЦИЯ =============
    'r': 0.57,           # Коэффициент рециркуляции (57%)
                         # Обеспечивает стабильный возврат ила
    
    'X_r_max': 8000,     # Макс. концентрация ила в рецикле, мг/л
                         # Ограничение на уплотнение ила в отстойнике
    
    'eta': 0.3,          # Удаление инертных в отстойнике (30%)
                         # Консервативная оценка для запаса прочности
    
    # ============= АДАПТАЦИЯ =============
    'tau_adapt': 36,     # Время адаптации бактерий, ч (1.5 дня)
                         # Обеспечивает плавный выход на режим
}



# Запуск
sol, params = aeration_tank_physical(params_vlg)

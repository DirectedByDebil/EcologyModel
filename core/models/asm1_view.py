...
def plot_simple(sol, params, clarifier_results):
    t = sol.t
...
def plot_with_nitri(sol, params, clarifier_results):
    """
    ...
    S_aerotenk = S_bio + S_inert
    X_total = X_BH + X_BA  # общая биомасса (гетеротрофы + автотрофы)
    ...
    # используем рассчитанное значение из отстойника, если есть
    ...
    # ============= 5. Биомасса (гетеротрофы + автотрофы) =============
    ...
    # ============= 7. Азот (аммоний и нитраты) =============
    # создаём новый макет, если нужно больше 6 графиков
...
def plot_results_v2(sol, params, clarifier_results, ctx = None):
    ...
    match ctx['model']:
        case 'simple':
            plot_simple(sol, params, clarifier_results)

        case 'with_nitri':
            plot_with_nitri(sol, params, clarifier_results)
